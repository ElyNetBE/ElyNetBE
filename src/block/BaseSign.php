	...
	public function onInteract(Item $item, int $face, Vector3 $clickVector, ?Player $player = null) : bool{
		if($player !== null){
			if($item instanceof Dye){
				...
				$ev = new SignChangeEvent($this, $player, new SignText($this->text->getLines(), $color, $this->text->isGlowingText()));
				$ev->call();
				if(!$ev->isCancelled()){
				...
				if($this->text->isGlowingText()){
					$ev = new SignChangeEvent($this, $player, new SignText($this->text->getLines(), $this->text->getBaseColor(), false));
					$ev->call();
					if(!$ev->isCancelled()){
						$this->text = $ev->getNewText();
						$this->position->getWorld()->setBlock($this->position, $this);
