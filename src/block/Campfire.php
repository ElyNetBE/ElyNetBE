		...
		if(!$this->extinguished){
			foreach($this->items as $slot => $item){
				...
				if($this->getSlotTime($slot) >= $this->getFurnaceType()->getCookDurationTicks()){
					$this->setItem(ItemFactory::air(), $slot);
					$this->setSlotTime($slot, 0);
					$result = ($item = $this->position->getWorld()->getServer()->getCraftingManager()->getFurnaceRecipeManager($this->getFurnaceType())->match($item)) instanceof FurnaceRecipe ? $item->getResult() : ItemFactory::air();
